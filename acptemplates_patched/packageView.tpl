...
<fieldset>
	...
	{if $package->getParentPackageID()}
		<div class="formElement">
			...
			<p class="formField"><a href="index.php?page=PackageView&amp;activePackageID={@$package->getParentPackageID()}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{$package->getParentPackage()->getName()}</a></p>
		</div>
	{/if}
	<div class="formElement">
...
<div class="contentHeader">
	<div class="largeButtons">
		<ul>
			{if PACKAGE_ID != $package->getPackageID()}
				{if $package->isStandalone() && $package->getPackage() != 'com.woltlab.wcf'}<li><a href="{@RELATIVE_WCF_DIR}{$package->getDir()}acp/index.php?packageID={@$package->getPackageID()}{@SID_ARG_2ND}" title="{lang}wcf.acp.package.view.button.makeActive{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/packageMakeActiveM.png" alt="" /> <span>{lang}wcf.acp.package.view.button.makeActive{/lang}</span></a></li>{/if}
				{if $this->user->getPermission('admin.system.package.canUninstallPackage') && $noDependentIsActive}<li><a href="index.php?page=Package&amp;action=startUninstall&amp;activePackageID={@$package->getPackageID()}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}" onclick="return confirm('{lang}wcf.acp.package.view.button.uninstall.sure{/lang}')" title="{lang}wcf.acp.package.view.button.uninstall{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/packageUninstallM.png" alt="" /> <span>{lang}wcf.acp.package.view.button.uninstall{/lang}</span></a></li>{/if}
			{/if}
			{if $this->user->getPermission('admin.system.package.canUpdatePackage')}<li><a href="index.php?form=PackageStartInstall&amp;action=update&amp;activePackageID={@$package->getPackageID()}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}" title="{lang}wcf.acp.package.view.button.update{/lang}"><img src="{@RELATIVE_WCF_DIR}icon/packageUpdateM.png" alt="" /> <span>{lang}wcf.acp.package.view.button.update{/lang}</span></a></li>{/if}
			{if $hasLicenseText|isset}<li><a href="index.php?page=PackageShowLicenseText&amp;activePackageID={@$package->getPackageID()}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}"><img src="{@RELATIVE_WCF_DIR}icon/packageLicenseTextM.png" alt="" /> <span>{lang}wcf.acp.package.view.button.showlicensetext{/lang}</span></a></li>{/if}
		</ul>
	</div>
</div>
		...
		<table class="tableList">
			...
			{foreach from=$requiredPackages item=$package}
				<tr class="{cycle values="container-1,container-2"}">
					...
					<td class="columnText" title="{$package.packageDescription}"><a href="index.php?page=PackageView&amp;activePackageID={@$package.packageID}&amp;packageID={@PACKAGE_ID}{@SID_ARG_2ND}">{lang}wcf.acp.package.list.packageName{/lang}</a></td>
